...
async function main() {
	console.log(chalk.bold.blue("Starting Protocol Buffer code generation using buf..."))
	...
	// Process all proto files using buf generate
	...
	// Use buf generate instead of protoc
	await bufGenerate(TS_OUT_DIR, protoFiles, TS_PROTO_OPTIONS)
	await bufGenerate(GRPC_JS_OUT_DIR, protoFiles, ["outputServices=grpc-js,outputClientImpl=false", ...TS_PROTO_OPTIONS])
	await bufGenerate(NICE_JS_OUT_DIR, protoFiles, ["outputServices=nice-grpc,useExactTypes=false", ...TS_PROTO_OPTIONS])

	// Generate descriptor set using buf
	const descriptorFile = path.join(DESCRIPTOR_OUT_DIR, "descriptor_set.pb")
	try {
		log_verbose(chalk.cyan("Generating descriptor set using buf..."))
		execSync(`npx buf build --output "${descriptorFile}"`, {
			cwd: ROOT_DIR,
			stdio: "inherit",
		})
	} catch (error) {
		console.error(chalk.red("Error generating descriptor set:"), error)
		process.exit(1)
	}
...
async function bufGenerate(outDir, protoFiles, protoOptions) {
	// Create a temporary buf.gen.yaml for this specific generation
	const bufGenConfig = {
		version: "v2",
		plugins: [
			{
				...
				out: outDir,
				opt: protoOptions.join(","),
			},
		],
	}

	const tempConfigFile = path.join(ROOT_DIR, `buf.gen.${path.basename(outDir)}.yaml`)

	try {
		log_verbose(chalk.cyan(`Generating TypeScript code in ${outDir}...`))
...
`,
		...
		// Run buf generate
		execSync(`npx buf generate --template "${tempConfigFile}"`, {
			cwd: ROOT_DIR,
			stdio: "inherit",
		})
